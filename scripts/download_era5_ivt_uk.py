"""
Download ERA5 IVT (integrated vapour transport) for UK SW coast (50.0N, 5.0W)
via the FULL single-levels dataset (timeseries API does NOT serve viwve/viwvn).
Small area box, netCDF, 2-year chunks, 6-hourly (00/06/12/18), 1980-2023.
IVT magnitude = sqrt(viwve^2 + viwvn^2)  (order-independent, so var naming is moot).
Same approach that produced era5_ivt_ca_*.nc for California.

Output:
  - data/raw/era5_ivt_uk_<y0>_<y1>.nc   (raw chunks, cached/resumable)
  - data/raw/ivt_uk_1980_2023.npy       (6-hourly IVT magnitude at nearest cell)
  - data/raw/times_uk_1980_2023.npy     (6-hourly timestamps, for circ alignment)

Run: python scripts/download_era5_ivt_uk.py            (full run, resumable)
     python scripts/download_era5_ivt_uk.py --test     (first chunk only, sanity)
...
# UK SW coast (Cornwall). Retarget by editing LAT/LON/TAG/AREA (e.g. Chile -33.0/-71.5).
LAT, LON = 50.0, -5.0
TAG = "uk"
AREA = [50.5, -5.5, 49.5, -4.5]   # N, W, S, E  (small box around the point)
VARS = [
...
CHUNKS = [(y, y + 1) for y in range(1980, 2024, 2)]   # (1980,1981) .. (2022,2023) = 22 chunks
...
def build():
    ...
    order = np.argsort(t)               # ensure chronological
