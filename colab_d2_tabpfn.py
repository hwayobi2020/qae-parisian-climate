...
def raw_preds(fcv, yf, odf, thr):                       # 기준: 예보 min IVT >= THR
    yt = []; pb = []
    for tr, te in folds(len(yf), odf):
        if len(tr) < 40 or len(np.unique(yf[tr])) < 2: continue
        pb.extend((fcv[te] >= thr).astype(int)); yt.extend(yf[te])
    return np.array(yt), np.array(pb)


def reg_preds(X, omin, yf, odf, thr):                   # 우리모델: 예보피처->관측 min IVT 회귀예측 >= THR
    yt = []; pb = []
    for tr, te in folds(len(yf), odf):
        if len(tr) < 40 or len(np.unique(yf[tr])) < 2: continue
        m = TabPFNRegressor(device=DEV); m.fit(np.nan_to_num(X[tr]), omin[tr])
        pred = m.predict(np.nan_to_num(X[te]))
        pb.extend((np.asarray(pred) >= thr).astype(int)); yt.extend(yf[te])
    return np.array(yt), np.array(pb)
...
for R in ["ca", "uk", "chile"]:
    ...
    yt0, p0 = raw_preds(fcv, y_s, od_s, THR); f0 = f1_score(yt0, p0, zero_division=0)
    yt1, p1 = reg_preds(D2s, omin_s, y_s, od_s, THR); f1v = f1_score(yt1, p1, zero_division=0)
    lo, hi, pp = boot(yt0, p1, p0)
    print(f"  기준 raw F1={f0:.3f} | 우리모델(회귀) F1={f1v:.3f} | ΔF1={f1v - f0:+.3f} 95%CI[{lo:+.3f},{hi:+.3f}] P(Δ>0)={pp:.3f}")
    REG[R] = (yt0, p0, p1)
