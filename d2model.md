...
### 입력 피처 12개 (메인 피처셋 = C. 전부 GEFS 컨트롤 멤버 c00 한 개의 예보 IVT에서 나옴)

| 번호 | 이름 | 내용 |
|---|---|---|
| 1~8 | 고정 리드 원값 8개 (= **D8**) | 발표 시점(D−2일 00시)부터 48·54·60·66·72·78·84·90시간 뒤, 8개 시점의 예보 IVT 값을 그대로 넣는다. 어떤 날이든 피처 1번은 항상 "발표+48시간" 값 — 날마다 시점을 밀지 않는다(정렬 없음). |
| 9~12 | 지속창 요약 4개 | 기준점 = **예보 peak**(발표 후 48~66시간 중 예보 IVT가 최대인 시각, 코드명 onL). 그 뒤 +6/+12/+18시간 세 값으로 **최소값(min) / 평균(mean) / 표준편차(std)** 를 만들고, **기울기**(peak+24시간 값 − peak 값)를 더한다. |

- 용어: **fcv = forecast value(예보값)** — 위 요약 중 "최소값"과 같은 양. 원예보(raw) 기준선이 지속 판정에 쓰는 바로 그 값이다(§4).
- **이전 버전 = D2 (peak정렬 9개)**: 예보 peak 기준 궤적 5점(peak·+6·+12·+18·+24h) + 요약 4개. 2026-07-07 ablation에서 고정리드 방식보다 열등 확인(예: 18h TabPFN CA ΔF1 +0.074→+0.117, UK +0.029→+0.058) → 메인을 C로 교체. ("온셋"은 라벨을 만들 때만 쓰는 개념이고 피처가 아니다 — 피처는 전부 예보에서만 나온다.)
- **⚠️ 진행 중 (2026-07-19)**: §8 등 이 문서의 헤드라인 결과는 아직 이전 버전 D2로 뽑은 것 → 메인 피처로 재산출 필요. 또한 **B(고정 8개만) vs C(고정 8개+요약 4개)** 가 horizon마다 엇갈려(24h TabPFN은 B 우세, 18h·30h는 C 우세) 둘의 차이를 직접 검정하는 C−B paired 부트스트랩을 추가함(`colab_align_ablation.py`) — 결과 대기. 데이터 생성 = `build_op_denom_full.py`.
