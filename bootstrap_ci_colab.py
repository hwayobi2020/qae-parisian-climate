...
print(f"n={n}, train={i1} test={n-i2}, 부트스트랩 B={B}\n")
for thr in [2,3,4,5]:
    ...
    P=np.column_stack([fp(mk,Xtr,ytr,Xte) for mk in makers]); avg=P.mean(1)
    cols={**{names[k]:P[:,k] for k in range(3)}, "평균":avg}
    aucs={nm:[] for nm in cols}; diffs={nm:[] for nm in names}
    for _ in range(B):
        idx=rng.randint(0,len(yte),len(yte))
        if len(np.unique(yte[idx]))<2: continue
        a={nm:roc_auc_score(yte[idx],cols[nm][idx]) for nm in cols}
        for nm in cols: aucs[nm].append(a[nm])
        for nm in names: diffs[nm].append(a["평균"]-a[nm])
    print(f"[>= {thr}일]  양성 test={int(yte.sum())}/{len(yte)}")
    for nm in cols:
        v=np.array(aucs[nm]); print(f"  {nm:>7}  AUC={np.median(v):.3f}  95%CI[{np.percentile(v,2.5):.3f}, {np.percentile(v,97.5):.3f}]")
    for nm in names:
        d=np.array(diffs[nm]); print(f"  평균-{nm:<6} 차이 {np.median(d):+.3f}  95%CI[{np.percentile(d,2.5):+.3f}, {np.percentile(d,97.5):+.3f}]  P(>0)={(d>0).mean():.2f}")
    print()
